<template>
    <div>
        ...
        <div v-else>
            <div class="flex justify-center mt-2 mx-5" id="resume-element">
            ...
            <div class="flex justify-center mt-3">
                <button @click="generatePDF" class="btn btn-outline btn-primary btn-sm">Save to PDF</button>
            </div>
        </div>
    </div>
</template>
...
onMounted(async () => {
    ...
    imagePath.value = `http://localhost/storage/${resume.value.image_path}`;
});

function generatePDF() {
    const element = document.getElementById('resume-element')
    html2pdf()
        .from(element)
        .save()
}
